...
class mime_decode {
	...
	function get_names($strmail) {
		$ARfrom = Array();
		$strmail = stripslashes(preg_replace("/\t/","",preg_replace("/\n/","",preg_replace("/\r/","",$strmail))));
		...
		for($i=0;$i<count($armail);$i++) {
			$thisPart = trim(preg_replace("/^\"(.*)\"$/", "\\1", trim($armail[$i])));
			if($thisPart != "") {
				if (preg_match("/(.*)<(.*)>/", $thisPart, $regs)) {
					$email = trim($regs[2]);
					$name = trim($regs[1]);
				} else {
					if (preg_match("/([-a-z0-9_$+.]+@[-a-z0-9_.]+[-a-z0-9_]+)((.*))/", $thisPart, $regs)) {
						$email = $regs[1];
						$name = $regs[2];
					} else
						$email = $thisPart;
	...
	function make_link_clickable($text){
		$text = preg_replace("/([[:alnum:]]+)://([^[:space:]]*)([[:alnum:]#?/&=])/",
			"<a class=\"autolink\" href=\"\\1://\\2\\3\" target=\"_new\">\\1://\\2\\3</a>", $text);
		$text = preg_replace("/([0-9a-z]([-_.]?[0-9a-z])*@[0-9a-z]([-.]?[0-9a-z])*\\.[a-z]{2,3})/","<a class=\"autolink\"  href=\"newmsg.php?mailto=\\1&nameto=\\1\">\\1</a>", $text);
		return $text;
	}
	...
	function build_attach($header,$body,$boundary,$part) {
		...
		if($Atype[0] == "message") {
			...
			if($filename == "")
				$filename = uniqid("");
			$filename = substr(preg_replace("/[^A-Za-z0-9]/","_",$filename),0,20).".eml";
		} else {
			$fname = $Acdisp[1];
	...
	function download_attach($header,$body,$down=1) {
		...
		$type = explode(";",$ctype); $type = $type[0];
		$Atype = explode("/",$ctype);
		$Acdisp = explode(";",$cdisp);
		...
		if($Atype[0] == "message") {
			...
			if($filename == "")
				$filename = uniqid("");
			$filename = substr(preg_replace("/[^A-Za-z0-9]/","_",$filename),0,20);
			$filename .= ".eml";
		} else {
			$fname = $Acdisp[1];
	...
	function get_boundary($ctype){
		...
		$boundary = explode(";",$boundary);$boundary = $boundary[0];
