...
class phpmailer {
	...
	var $blUseAuthLogin = true;
	//var $AuthUser       = "051101";
	//var $AuthPass       = "Telkomd5";
	var $AuthUser       = "survey_online";
	var $AuthPass       = "telkomsurvey";
	...
	function Start() {
		...
		$this->Helo = preg_replace("/[^A-Za-z0-9]/","",$appname);
	}
	...
	function smtp_send($header, $body) {
		...
		if($this->blUseAuthLogin) {
			if(!$smtp->AuthHello($this->Helo,$this->AuthUser,$this->AuthPass)) {
			   $this->error_handler("SMTP Error: Inválid username/password ...");
			   return false;
			}
		} else
...
class SMTP {
    ...
    function send_line($data) {
		fputs($this->smtp_conn,$data.$this->CRLF);
		if($this->do_debug >= 4)
			echo "SMTP -> send_line(): \$data is \"$data\"" . $this->CRLF;
    }
