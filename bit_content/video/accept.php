...
<table width="100%" border="0">
	<tr>
		<td colspan="2">
			<? echo setTitle2("Approve / Reject Video");?>
		</td>
	</tr>
	<tr>
...
<?
	...
	if ($_POST["sRej"]) {
		...
		if (!$ora->sql_no_fetch($qry,$bit_app["db"])) {
			alert('Reject Forum gagal, mohon hubungi admin !');
			$ora->logoff();
		} else {
			
			$qry="select title,updated_by,publisher from m_video where id=".$_GET["id"];
			$rsData=$ora->sql_fetch($qry,$bit_app["db"]);
			
			#Mail
			$to=$rsData->value[1][2]."@telkom.co.id";
			$subject="[ Movie Web Infratel ] Rejected : ".$rsData->value[1][1];
			$info["author"]=getUser($rsData->value[1][2]);
			$info["judul"]=$rsData->value[1][1];
			$info["note"]=$_POST["tInput"];
			send_email("reject",$to,$subject,$info,"Movie");
	...
	if ($_POST["sApr"]) {
		...
		$qry="update m_video
				...
				where
					id=".$_GET["id"];
		if (!$ora->sql_no_fetch($qry,$bit_app["db"])) {
			alert('Approve Forum gagal, mohon hubungi admin !');
			$ora->logoff();
		} else {
			$qry="select title,updated_by,publisher,keterangan from m_video where id=".$_GET["id"];
			$rsData=$ora->sql_fetch($qry,$bit_app["db"]);
			
			#Mail
			$to=$rsData->value[1][2]."@telkom.co.id";
			$subject="[ Movie Web Infratel ] Telah di publish : ".$rsData->value[1][1];
			$info["author"]=getUser($rsData->value[1][2]);
			$info["judul"]=$rsData->value[1][1];
			$info["note"]=$_POST["tInput"];
			send_email("approve",$to,$subject,$info,"Movie");
			
			$to=$bit_app["milis"];
			$subject="[ Video Web Infratel ] HOT VIDEO : ".$rsData->value[1][1];
			$info["author"]=getUser($rsData->value[1][2]);
			$info["judul"]=$rsData->value[1][1];
			$info["description"]=$rsData->value[1][4];
			send_email("milis",$to,$subject,$info,"Video");
