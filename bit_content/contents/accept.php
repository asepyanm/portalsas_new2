...
<script>
	function validate() {
		if (document.forms[0].tInput.value=='') {
			alert('Pesan diisi terlebih dahulu !');
...
<table width="95%" border="0">
	<tr>
		<td colspan="2">
			<? echo setTitle2("Approve / Reject News");?>
		</td>
	</tr>
	<tr>
...
<?
	...
	if ($_POST["sRej"]) {
		...
		if (!$ora->sql_no_fetch($qry,$bit_app["db"])) {
			alert('Reject Content gagal, mohon hubungi admin !');
			$ora->logoff();
		} else {
			$qry="select title,created_by,publisher from m_contents where id=".$_GET["id"];
			$rsData=$ora->sql_fetch($qry,$bit_app["db"]);
			
			$to=$rsData->value[1][2]."@telkom.co.id";
			$subject="[ News Web Infratel ] Rejected : ".$rsData->value[1][1];
			$info["author"]=getUser($rsData->value[1][2]);
			$info["judul"]=$rsData->value[1][1];
			$info["note"]=$_POST["tInput"];
			send_email("approve",$to,$subject,$info,"News");
	...
	if ($_POST["sApr"]) {
		...
		$qry="update m_contents
				...
				where
					id=".$_GET["id"];
		if (!$ora->sql_no_fetch($qry,$bit_app["db"])) {
			alert('Approve News gagal, mohon hubungi admin !');
			$ora->logoff();
		} else {
			$qry="select title,created_by,publisher,content from m_contents where id=".$_GET["id"];
			$rsData=$ora->sql_fetch($qry,$bit_app["db"]);
			
			$to=$rsData->value[1][2]."@telkom.co.id";
			$subject="[ News Web Infratel ] Telah di publish : ".$rsData->value[1][1];
			$info["author"]=getUser($rsData->value[1][2]);
			$info["judul"]=$rsData->value[1][1];
			$info["note"]=$_POST["tInput"];
			send_email("approve",$to,$subject,$info,"News");
			...
			$to=$bit_app["milis"];
			$subject="[ News Web Infratel ] HOT NEWS : ".$rsData->value[1][1];
			$info["author"]=getUser($rsData->value[1][2]);
			$info["judul"]=$rsData->value[1][1];
			$info["description"]=$rsData->value[1][4];
			send_email("milis",$to,$subject,$info,"News");
