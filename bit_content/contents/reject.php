...
<?
	...
	$qry="select count(1) cnt,1 from m_contents a where created_by='".getUserID()."' and publish_flag=0 and status=2
			union
			select count(1) cnt,6 from m_video d where created_by='".getUserID()."' and publish_flag=0 and status=2
			union
			select count(1) cnt,3 from m_foto c where created_by='".getUserID()."' and publish_flag=0 and status=2
			";
	...
	<tbody>
		<?
			...
			$qry="select 
					count(1) 
				  from $tbl
				  where created_by='".getUserID()."' and publish_flag=0 and status=2 $where";
		...
		?>			
				<tr class="<? echo $cl?>">
					...
					<td width="1%" align="center">
						<img title="Submit" style="cursor:pointer" src="<? echo $bit_app["path_url"]?>/bit_images/tick.png" onClick="var win; win=window.open('<? echo $bit_app["path_url"]."/bit_content/".$folder."/"?>submit.php?id=<? echo $rs->value[$i][1]?>','win','top=200,left=300,height=190,width=500,resizable=1,scrollbars=1');win.focus()">
					</td>
				</tr>	
		<? } ?>
	</tbody>
