...
<?
	...
	case 1 : 
		...
		$title="News";
		$folder="contents";
		$qry="select nik from p_publisher_news where loker2='".getUserLoker()."'";
		break;
	case 2 : 
		$tbl="m_info";
	...
	case 8 : 
		...
		$folder="infra";
		$qry="select nik from p_publisher_infra";
		break;
	case 9 : 
		$tbl="m_inovasi";
		$title="Inovasi";
	...
	$arrPublisher=explode(",",$rsPublisher->value[1][1]);
	$tmpPublisher ="";
	for ($i=0;$i<count($arrPublisher);$i++) {
		$tmpPublisher .=getUser2($arrPublisher[$i])."<br >";
	}
	
	$qry="select count(1) cnt,1 from m_contents a where created_by='".getUserID()."' and publish_flag=0 
			union
			select count(1) cnt,6 from m_video d where created_by='".getUserID()."' and publish_flag=0 
			union
			select count(1) cnt,3 from m_foto c where created_by='".getUserID()."' and publish_flag=0 
		";
...
?>
	...
	<tr>
		<td colspan="5">
			<? echo setTitle2("Menunggu Approve $title");?>
		</td>
	</tr>
	</table>
	<table cellpadding="1" cellspacing="0" border="0" width="90%">
		<tr>
			...
			<td rowspan="2" class="bit_publisher">
			<b>Publisher : </b><br />
			<? echo $tmpPublisher ?></td>
		</tr>
		<tr>
			<td>Judul </td>
			<td align="left">
	...
	<tbody>
		<?
			...
			$qry="select 
					count(1) 
				  from $tbl
				  where created_by='".getUserID()."' and publish_flag=0 $where";
		...
		?>			
				<tr class="<? echo $cl?>">
					...
					<td width="1%" align="center">
						<img title="Edit News" style="cursor:pointer" src="<? echo $bit_app["path_url"]?>/bit_images/edit.gif" onClick="var win; win=window.open('<? echo $bit_app["path_url"]."/bit_content/".$folder."/"?>edit.php?id=<? echo $rs->value[$i][1]?>','win','top=10,left=100,height=650,width=600,resizable=1,scrollbars=1');win.focus()">
					</td>
					<td width="1%" align="center">
					...
					<td width="1%" align="center">
						<img title="Hapus News" style="cursor:pointer" src="<? echo $bit_app["path_url"]?>/bit_images/hapus.png" onClick="var ans;ans=confirm('Anda yakin untuk menghapus [<? echo $rs->value[$i][2]?>] ?'); if (ans) {document.forms[0].hdAction.value=3;document.forms[0].id.value=<? echo $rs->value[$i][1]?>;document.forms[0].submit();}">
					</td>
