...
<?
	...
	$qry="select count(1) cnt,1 from m_contents a where created_by='".getUserID()."' and publish_flag=1
			union
			select count(1) cnt,6 from m_video d where created_by='".getUserID()."' and publish_flag=1
			union
	...
	<tbody>
		<?
			...
			$qry="select 
					count(1) 
				  from $tbl
				  where created_by='".getUserID()."' and publish_flag=1 $where";
