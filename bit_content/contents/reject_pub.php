...
<?
	...
	$qry="select count(1) cnt,1 from m_contents a where reject_by_info='".getUser(getUserID())."' and publish_flag=0 and status=2
			union
			select count(1) cnt,6 from m_video d where reject_by_info='".getUser(getUserID())."' and publish_flag=0 and status=2
			union
	...
	<tbody>
		<?
			...
			$qry="select 
					count(1) 
				  from $tbl
				  where reject_by_info='".getUser(getUserID())."' and publish_flag=0 and status=2 $where";
