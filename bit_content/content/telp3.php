<?
	...
	if ($_POST["ok"] && $_FILES["tFile"]["error"]==0) {
		...
		#$qry="delete from m_telp_pejabat_penting";
		...
		while ($objPHPExcel->getActiveSheet()->getCellByColumnAndRow(0,$i)->getvalue()) {
			...
			$qry="select * from m_telp_pejabat_penting where nama='".$nama."'";
			$rsC=$ora->sql_fetch($qry,$bit_app["db"]);;
			if ($rsC->jumrec<=0) {
				$qry="insert into m_telp_pejabat_penting ($field) values(".$valueRow.")";
				$ora->sql_no_fetch($qry,$bit_app["db"]);
			} else {
				$errorMsg .="<font color='#FF0000'>data untuk nama instansi <b>".$nama."</b> sudah ada di database !</font><br />";
			}
...
?>
	...
	<tr>
		<td colspan="5">
			<? echo setTitleCari("Nomor Telepon Penting");?>
		</td>
	</tr>
	<tr>
		<td>	
			...
			<a title="Contoh Template Excel" href="content/template/telp_pejabat_penting.xls" target="_blank">download contoh template excel</a>
			<? echo "<br /><br />".$errorMsg; ?>
		</td>
	</tr>
	</table>
	<table cellpadding="1" cellspacing="0">
		<tr>
			<td align="right">
				...
				(Pencarian terhadap Nama,Alamat,Telepon)
			</td>
		</tr>
	</table>
	...
	<thead>
		<tr>
			...
			<th rowspan="2" class="listTableHead">NAMA INSTANSI</th>	
			<th rowspan="2" class="listTableHead">ALAMAT</th>	
	...
	<tbody>
		...
		?>			
				<tr class="<? echo $cl?>">
					...
					<td align="left" width="40%" valign="top"><? echo $rs->value[$i][3]?></td>
					<td align="center" width="5%" nowrap="nowrap" valign="top"><? echo $rs->value[$i][4]?></td>
					<td align="center" width="5%" nowrap="nowrap" valign="top"><? echo $rs->value[$i][5]?></td>
					<td width="1%" align="center">
						<img style="cursor:pointer" title="Edit Data" src="<? echo $bit_app["path_url"]?>/bit_images/edit.gif" onClick="var win; win=window.open('<? echo $bit_app["path_url"]."/bit_content/content/"?>telp3Edit.php?id=<? echo $rs->value[$i][1]?>','win','top=10,left=100,height=320,width=600,resizable=1,scrollbars=1');win.focus()">
					</td>
					<td width="1%" align="center">
	...
	<p align="right">
	<? $f->button("add","Tambah","button","var win;win=window.open('".$bit_app["path_url"]."/bit_content/content/telp3Add.php','win','top=10,left=100,height=320,width=600,resizable=1,scrollbars=1');win.focus()")?>
	&nbsp;
