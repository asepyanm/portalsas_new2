...
?>
	...
	<table cellpadding="1" cellspacing="0" class="table">
		...
		<tr>
			...
			<td align="left">
				: <?
					$qry="select created_by_info,count(1) from m_foto group by created_by_info";
		...
		<tr>
			...
			<td align="left">
				: <?
					$qry="select publish_by_info,count(1) from m_foto where publish_by_info is not null group by publish_by_info";
					$rsFilter=$ora->sql_fetch($qry,$bit_app["db"]);
	...
	<br />
		<div><a href='#' onClick="showColorbox('<? echo $bit_app["path_url"] ?>bit_content/foto/add.php');">Upload Foto</a></div>
	<br />

	<table cellpadding="0" cellspacing="1" border="0" width="100%">
	<thead>
		<tr>
		<? $arrField=array("No","Judul Foto","Hit","Tgl.Submit","Author","Tgl.Publish","Publisher","Owner") ?>
		<?
		...
		?>			
				<tr class="<? echo $cl?>">
					...
					<td width="1%" align="center">	
						<img style="cursor:pointer" title="Edit Foto" style="cursor:pointer" src="<? echo $bit_app["path_url"]?>/bit_images/edit.gif" onClick="var win; win=window.open('<? echo $bit_app["path_url"]."/bit_content/content/"?>fotEdit.php?id=<? echo $rs->value[$i][1]?>','win','top=10,left=100,height=550,width=600,resizable=1,scrollbars=1');win.focus()">
					</td>
					<td width="1%" align="center">
						<img style="cursor:pointer" title="Hapus Foto" style="cursor:pointer" src="<? echo $bit_app["path_url"]?>/bit_images/hapus.png" onClick="var ans;ans=confirm('Anda yakin untuk menghapus Foto [<? echo $rs->value[$i][2]?>] ?'); if (ans) {document.forms[0].hdAction.value=3;document.forms[0].id.value=<? echo $rs->value[$i][1]?>;document.forms[0].submit();}">
					</td>
					<td width="1%" align="center">
						<img style="cursor:pointer" title="Detail Foto" style="cursor:pointer" src="<? echo $bit_app["path_url"]?>/bit_images/detail.gif" onClick="var win; win=window.open('<? echo $bit_app["path_url"]."/bit_content/foto/"?>det.php?id=<? echo $rs->value[$i][1]?>','win','top=10,left=100,height=650,width=600,resizable=1,scrollbars=1');win.focus()">
					</td>
				</tr>	
		<? } ?>
	</tbody>
