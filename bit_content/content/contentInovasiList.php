...
?>
	...
	<table cellpadding="1" cellspacing="0">
		...
		<tr>
			...
			<td align="left">
				: <?
					$qry="select created_by_info,count(1) from m_inovasi group by created_by_info";
		...
		<tr>
			...
			<td align="left">
				: <?
					$qry="select publish_by_info,count(1) from m_inovasi where publish_by_info is not null group by publish_by_info";
					$rsFilter=$ora->sql_fetch($qry,$bit_app["db"]);
	...
	<tbody>
		<?
			...
			case 3 :
				$qry="delete from m_inovasi
						where id=".(int)$_POST["id"];
				$ora->sql_no_fetch($qry,$bit_app["db"]); break;
			case 4 :
				$qry="update m_inovasi
		...
		?>			
				<tr class="<? echo $cl?>">
					...
					<td width="1%" align="center">
						<img title="Edit Inovasi" style="cursor:pointer" src="<? echo $bit_app["path_url"]?>/bit_images/edit.gif" onClick="var win; win=window.open('<? echo $bit_app["path_url"]."/bit_content/inovasi/"?>edit.php?id=<? echo $rs->value[$i][1]?>','win','top=10,left=100,height=550,width=600,resizable=1,scrollbars=1');win.focus()">
					</td>
					<td width="1%" align="center">
						<img title="Hapus Inovasi" style="cursor:pointer" src="<? echo $bit_app["path_url"]?>/bit_images/hapus.png" onClick="var ans;ans=confirm('Anda yakin untuk menghapus Content [<? echo $rs->value[$i][2]?>] ?'); if (ans) {document.forms[0].hdAction.value=3;document.forms[0].id.value=<? echo $rs->value[$i][1]?>;document.forms[0].submit();}">
					</td>
					<td width="1%" align="center">
						<img title="Detail Inovasi" style="cursor:pointer" src="<? echo $bit_app["path_url"]?>/bit_images/detail.gif" onClick="var win; win=window.open('<? echo $bit_app["path_url"]."/bit_content/inovasi/"?>det.php?id=<? echo $rs->value[$i][1]?>','win','top=10,left=100,height=650,width=600,resizable=1,scrollbars=1');win.focus()">
					</td>
				</tr>	
		<? } ?>
	</tbody>
