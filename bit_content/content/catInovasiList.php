...
?>
	...
	<tr>
		<td colspan="5">
			<? echo setTitleCari("Daftar Kategori Inovasi");?>
		</td>
	</tr>
	</table>
	...
	<tbody>
		<?
			...
			//=============================================
			$qry="select 
					category_id,category_name,registered
				  from p_category_inovasi a
				  where 1=1 $where
				  $orderBy
		...
		?>			
				<tr class="<? echo $cl?>">
					...
					<td width="1%">
						<img title="Edit Kategori" style="cursor:pointer" src="<? echo $bit_app["path_url"]?>/bit_images/edit.gif" onClick="var win; win=window.open('<? echo $bit_app["path_url"]."/bit_content/content/"?>catInovasiEdit.php?id=<? echo $rs->value[$i][1]?>','win','top=10,left=100,height=250,width=600,resizable=1,scrollbars=1');win.focus()">
					</td>
					<td width="1%">
						<img title="Hapus Kategori" style="cursor:pointer" src="<? echo $bit_app["path_url"]?>/bit_images/hapus.png" onClick="var ans;ans=confirm('Anda yakin untuk menghapus Kategori [<? echo $rs->value[$i][2]?>] ?'); if (ans) {document.forms[0].hdAction.value=3;document.forms[0].id.value=<? echo $rs->value[$i][1]?>;document.forms[0].submit();}">
					</td>
				</tr>	
		<? } ?>
	</tbody>
	...
	<p align="right">
	<? $f->button("add","Tambah","button","var win;win=window.open('".$bit_app["path_url"]."/bit_content/content/catInovasiAdd.php','win','top=10,left=100,height=250,width=600,resizable=1,scrollbars=1');win.focus()")?>
	&nbsp;
