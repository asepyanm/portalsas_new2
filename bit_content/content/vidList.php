...
<body leftmargin="0" topmargin="0">
	...
	<table cellpadding="0" cellspacing="1" border="0" width="100%">
		<thead>
			<tr>
				<? $arrField = array("NO", "Keterangan", "video", "Tanggal", "Oleh") ?>
				<?
		...
		<tbody>
			...
			//=============================================
			$qry = "select 
					id,keterangan,file,date_format(updated_date,'%Y%m%d, %h:%i:%s') updated_date, updated_by
				  from m_video a
				  where 1=1 $where
				  $orderBy
			...
			?>
				<tr class="<? echo $cl ?>">
					...
					<td align="left" width="30%"><? echo $rs->value[$i][2] ?></td>
					<td align="left" width="15%"><? echo  $rs->value[$i][3] ?></td>
					<td align="left" width="10%" nowrap="nowrap"><? echo $rs->value[$i][4] ?></td>
					<td align="left" width="10%" nowrap="nowrap"><? echo $rs->value[$i][5] ?></td>
					<td width="1%" align="center">
						<img style="cursor:pointer" title="Edit Video" src="<? echo $bit_app["path_url"] ?>/bit_images/edit.gif" onClick="var win; win=window.open('<? echo $bit_app["path_url"] . "/bit_content/video/" ?>edit.php?id=<? echo $rs->value[$i][1] ?>','win','top=10,left=100,height=450,width=600,resizable=1,scrollbars=1');win.focus()">
					</td>
					<td width="1%" align="center">
						<img style="cursor:pointer" title="Delete Video" src="<? echo $bit_app["path_url"] ?>/bit_images/hapus.png" onClick="var ans;ans=confirm('Anda yakin untuk menghapus  ?'); if (ans) {document.forms[0].hdAction.value=3;document.forms[0].id.value=<? echo $rs->value[$i][1] ?>;document.forms[0].submit();}">
					</td>
					<td width="1%" align="center">
						<img style="cursor:pointer" title="Detail Video" src="<? echo $bit_app["path_url"] ?>/bit_images/detail.gif" onClick="var win; win=window.open('<? echo $bit_app["path_url"] . "/bit_content/video/" ?>det.php?id=<? echo $rs->value[$i][1] ?>','win','top=10,left=100,height=350,width=600,resizable=1,scrollbars=1');win.focus()">
					</td>
				</tr>
			<? } ?>
		</tbody>
	...
	<p align="right">
		<? $f->button("add", "Tambah", "button", "var win;win=window.open('" . $bit_app["path_url"] . "/bit_content/content/vidAdd.php','win','top=10,left=200,height=250,width=600,resizable=1,scrollbars=1');win.focus()") ?>
		&nbsp;
