...
?>
	...
	<thead>
		<tr>
		<? $arrField=array("NO","Tanggal","Event") ?>
		<?
	...
	<tbody>
		<?
			...
			$qry="select 
					id,keterangan,date_format(tgl_mulai,'%d %M %Y') tgl_mulai,date_format(tgl_selesai,'%d %M %Y') tgl_mulai
				  from m_calender a
				  where 1=1 $where
				  $orderBy
		...
		?>			
				<tr class="<? echo $cl?>">
					...
					<td align="left" width="30%">
						<? 
							if ($rs->value[$i][4])
								echo $rs->value[$i][3]." s/d ".$rs->value[$i][4];
							else
								echo $rs->value[$i][3];
						?>
					</td>
					<td align="left" width="15%"><? echo nl2br($rs->value[$i][2])?></td>
					<td width="1%" align="center">
						<img src="<? echo $bit_app["path_url"]?>/bit_images/edit.gif" onClick="var win; win=window.open('<? echo $bit_app["path_url"]."/bit_content/calendar/"?>edit.php?id=<? echo $rs->value[$i][1]?>','win','top=10,left=100,height=350,width=600,resizable=1,scrollbars=1');win.focus()">
					</td>
					<td width="1%" align="center">
	...
	<p align="right">
	<? $f->button("add","Tambah","button","var win;win=window.open('".$bit_app["path_url"]."/bit_content/content/calAdd.php','win','top=10,left=200,height=350,width=600,resizable=1,scrollbars=1');win.focus()")?>
	&nbsp;
