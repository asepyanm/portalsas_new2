...
?>
	...
	<thead>
		<tr>
		<? $arrField=array("NO","Judul","Banner","Tanggal") ?>
		<?
	...
	<tbody>
		<?
			...
			if ($_POST["tSearch"] && $_POST["tSearch"]!="Search") {
				$where .=" and (upper(description) like '%".strtoupper($_POST["tSearch"])."%')";
			}
			...
			//=============================================
			$qry="select 
					id,title,banner,description,date_format(created_date,'%d-%m-%Y')
				  from m_banner2 a
				  where 1=1 $where
				  $orderBy
		...
		?>			
				<tr class="<? echo $cl?>">
					...
					<td align="left" width="10%"><? echo $rs->value[$i][2]?></td>
					<td align="left" width="40%">
						<?  if ($rs->value[$i][3]) { ?>
							<object classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000" codebase="http://download.macromedia.com/pub/shockwave/cabs/flash/swflash.cab#version=7,0,19,0" width="50" height="75">
							<param name="movie" value="<? echo $bit_app["folder_url"].$rs->value[$i][3]?>" />
							<param name="quality" value="high" />
							<param name="wmode" value="transparent">
							<embed src="<? echo $bit_app["folder_url"].$rs->value[$i][3]?>" quality="high" pluginspage="http://www.macromedia.com/go/getflashplayer" type="application/x-shockwave-flash" width="50" height="75" wmode="transparent"></embed>
							</object>		
						<? } else { 
								echo nl2br($rs->value[$i][4]);
						   } ?>
					</td>
					<td align="left" width="10%"><? echo $rs->value[$i][5]?></td>
					<td width="1%" align="center">
						<img title="Hapus Banner" style="cursor:pointer" src="<? echo $bit_app["path_url"]?>/bit_images/hapus.png" onClick="var ans;ans=confirm('Anda yakin untuk menghapus Banner [<? echo $rs->value[$i][2]?>] ?'); if (ans) {document.forms[0].hdAction.value=3;document.forms[0].id.value=<? echo $rs->value[$i][1]?>;document.forms[0].submit();}">
					</td>
					<td width="1%" align="center">
						<img title="Edit Banner" style="cursor:pointer" src="<? echo $bit_app["path_url"]?>/bit_images/edit.gif" onClick="var win; win=window.open('<? echo $bit_app["path_url"]."/bit_content/banner/"?>edit2.php?id=<? echo $rs->value[$i][1]?>','win','top=10,left=100,height=400,width=600,resizable=1,scrollbars=1');win.focus()">
					</td>
				</tr>	
		<? } ?>
	</tbody>
	...
	<hr class="article_layout_hr">
	<p align="right">
	<? $f->button("add","Tambah","button","var win;win=window.open('".$bit_app["path_url"]."/bit_content/content/banner2Add.php','win','top=10,left=100,height=400,width=600,resizable=1,scrollbars=1');win.focus()")?>
