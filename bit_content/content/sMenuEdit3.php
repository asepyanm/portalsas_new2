...
<?
	...
	$qry="select sub_menu_name,tipe_content,content,target,banner from sub_menu_l3 where sub_menu_id=".$_GET["id"];
...
<table  width="100%">
	<tr >
		<td colspan="2">
			<? echo setTitleEdit("e d i t . s u b . m e n u");?>
		</td>
	</tr>
	<tr>
		<td width="30%">Nama Sub Menu (*)</td>
		<td>
			<? 
...
<?
	...
	if ($_POST["ok"]) {
		...
		$qry="update
					sub_menu_l3
				set	
					sub_menu_name='".$_POST["tInput"][0]."',
					tipe_content='".$_POST["rdInput"]."',
					target='".$_POST["sTarget"]."'
					$content
					$update
				where
					sub_menu_id=".(int)$_GET["id"];
		if (!$ora->sql_no_fetch($qry,$bit_app["db"])) {
			alert('Edit Sub Menu Gagal. Silahkan hubungi administrator !');
		} else {
			alert('Edit Sub Menu  berhasil !');
