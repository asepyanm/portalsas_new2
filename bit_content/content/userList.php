...
<body leftmargin="0" topmargin="0">
	...
	$qry = "select distinct upper(substring(b.nama_karyawan,1,1)) from m_users a, m_master_user b where a.user_id=b.nik order by substring(b.nama_karyawan,1,1)";
			...
			<tbody>
				...
				switch ($_POST["hdAction"]) {
					case 3:
						...
						$qry = "delete from m_master_user
						where nik=" . (int) $_POST["id"];
						$ora->sql_no_fetch($qry, $bit_app["db"]);
						break;
					case 2:
						$qry = "update m_users
				...
				$qry = "select 
					a.user_id,nama_karyawan,user_active_flag,user_level
				  from m_users a, m_master_user b
				...
				?>
					<tr class="<? echo $cl ?>">
						...
						<td>
							<img title="Edit User" style="cursor:pointer" src="<? echo $bit_app["path_url"] ?>bit_images/edit.gif" onClick="var win; win=window.open('<? echo $bit_app["path_url"] ?>bit_content/content/userEdit.php?id=<? echo $rs->value[$i][1] ?>','win','left=100,top=100,height=340,width=700,resizable=1,scrollbars=1')">
						</td>
						<td>
						...
						<td>
							<img title="Detail User" style="cursor:pointer" src="<? echo $bit_app["path_url"] ?>bit_images/detail.gif" onClick="var win; win=window.open('<? echo $bit_app["path_url"] ?>bit_content/content/userDet.php?id=<? echo $rs->value[$i][1] ?>','win','left=100,top=100,height=340,width=700,resizable=1,scrollbars=1')">
						</td>
						<? if ($rs->value[$i][3]) { ?>
							<td>
