...
?>
	...
	<a href='#' onClick="showColorbox('<? echo $bit_app["path_url"] ?>bit_content/contents/add.php?cat_id=<?php echo $cat?>');">Create News</a>

	<table cellpadding="0" cellspacing="1" border="0" width="100%" class="table">
	<thead>
		<tr>
		<?php $arrField=array("No","Judul Berita","Hit","Kategori","Tgl.Submit","Author","Tgl.Publish","Publisher","Owner") ?>
		<?php
		...
		?>			
				<tr class="<? echo $cl?>">
					...
					<td width="1%" align="center">
						...
						//-->
						<a href="#" onClick='showColorbox("<? echo $bit_app["path_url"]."/bit_content/contents/"?>edit.php?id=<? echo $rs->value[$i][1]?>");'><img title="Edit News" style="cursor:pointer" src="<? echo $bit_app["path_url"]?>/bit_images/edit.gif" /></a>
					</td>
					<td width="1%" align="center">
					...
					<td width="1%" align="center">
						<!--
							<img title="Detail News" style="cursor:pointer" src="<? echo $bit_app["path_url"]?>/bit_images/detail.gif" onClick="var win; win=window.open('<? echo $bit_app["path_url"]."/bit_content/contents/"?>det.php?id=<? echo $rs->value[$i][1]?>','win','top=10,left=100,height=650,width=600,resizable=1,scrollbars=1');win.focus()">
						//-->
						<a href="#" onClick='showColorbox("<? echo $bit_app["path_url"]."/bit_content/contents/"?>det.php?id=<? echo $rs->value[$i][1]?>");'><img title="Detail News" style="cursor:pointer" src="<? echo $bit_app["path_url"]?>/bit_images/detail.gif" /></a>
	...
	<a href='#' onClick="showColorbox('<? echo $bit_app["path_url"] ?>bit_content/contents/add.php?cat_id=<?php echo $cat?>');">Create News</a>
	<?php
//echo "<a onClick=\"$.fn.colorbox({iframe:true, width:'600px', height:'500px', inline:false, href:'bit_content/contents/add.php?cat_id=".$cat."'})\">Create News</a> ";
?>
<? $f->closeForm();
   $ora->logoff(); 
?>
...
{
	$.fn.colorbox({iframe:true, width:'90%', height:'90%', innerWidth:'90%', inline:false, scrolling:true, href: url});
}
*/
</script>
