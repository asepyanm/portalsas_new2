...
<script>
	function validate() {
		if (document.forms[0].tInput[0].value=='') {
			alert('Nama Sub Menu silahkan diisi terlebih dahulu !');
			document.forms[0].tInput[0].focus();
			return false;
		}
		
		if (document.forms[0].rdInput[0].checked) {
			if (document.forms[0].tInput[1].value=='') {
				alert('URL silahkan diisi terlebih dahulu !');
		...
		if (document.forms[0].rdInput[1].checked) {
			if (document.forms[0].tInput[1].value=='') {
				alert('Content silahkan diisi terlebih dahulu !');
				document.forms[0].tInput[1].focus();
				return false;
			}
...
<table width="100%">
	<tr >
		<td colspan="2">
			<? echo setTitleAdd("t a m b a h . s u b . m e n u");?>
		</td>
	</tr>
	<tr>
...
<?
	...
	if ($_POST["ok"]) {
		...
		$qry="insert into sub_menu_l3
				(sub_menu_name,menu_id,content,tipe_content,target)
				values(
				'".$_POST["tInput"][0]."',
				'".$_GET["id"]."',
				'".$_POST["tInput"][1]."',
		...
		if (!$ora->sql_no_fetch($qry,$bit_app["db"])) {
			alert('Tambah Sub Menu Gagal. Silahkan hubungi administrator !');
		} else {
			alert('Tambah Sub Menu  berhasil !');
