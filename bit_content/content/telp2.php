<?
	...
	$field="nama,nik,jabatan,loker,kantor,hp,flexi";
	...
	if ($_POST["ok"] && $_FILES["tFile"]["error"]==0) {
		...
		while ($objPHPExcel->getActiveSheet()->getCellByColumnAndRow(0,$i)->getvalue()) {
			...
			$qry="select * from m_telp_pejabat_telkom where nik='".$nik."'";
			$rsC=$ora->sql_fetch($qry,$bit_app["db"]);;
			if ($rsC->jumrec<=0) {
				$qry="insert into m_telp_pejabat_telkom ($field) values(".$valueRow.")";
				$ora->sql_no_fetch($qry,$bit_app["db"]);
			} else {
...
?>
	...
	<tr>
		<td colspan="5">
			<? echo setTitleCari("Nomor Telepon Pejabat Telkom");?>
		</td>
	</tr>
	<tr>
		<td>	
			...
			<a title="Contoh Template Excel" href="content/template/telp_pejabat_telkom.xls" target="_blank">download contoh template excel</a>
			<? echo "<br /><br />".$errorMsg; ?>
		</td>
	</tr>
	</table>
	...
	<tbody>
		<?
			...
			//=============================================
			$qry="select 
					id,nama,nik,jabatan,loker,kantor,hp,flexi
		...
		?>			
				<tr class="<? echo $cl?>">
					...
					<td align="center" nowrap="nowrap" width="5%" valign="top"><? echo $rs->value[$i][6]?></td>
					<td align="center" nowrap="nowrap" width="5%" valign="top"><? echo $rs->value[$i][7]?></td>
					<td align="center" nowrap="nowrap" width="5%" valign="top"><? echo $rs->value[$i][8]?></td>
					<td width="1%" align="center">
						<img style="cursor:pointer" title="Edit Data" src="<? echo $bit_app["path_url"]?>/bit_images/edit.gif" onClick="var win; win=window.open('<? echo $bit_app["path_url"]."/bit_content/content/"?>telp2Edit.php?id=<? echo $rs->value[$i][1]?>','win','top=10,left=100,height=320,width=600,resizable=1,scrollbars=1');win.focus()">
					</td>
					<td width="1%" align="center">
	...
	<p align="right">
	<? $f->button("add","Tambah","button","var win;win=window.open('".$bit_app["path_url"]."/bit_content/content/telp2Add.php','win','top=10,left=100,height=320,width=600,resizable=1,scrollbars=1');win.focus()")?>
	&nbsp;
