...
?>
	...
	<tr>
		<td colspan="5">
			<? echo setTitle2("Report Pengembangan Infrastruktur");?>
		</td>
	</tr>
	</table>
	<table cellpadding="1" cellspacing="0">
		...
		<tr>
			...
			<td align="left">
				: <?
					unset($arrAuthor);
					$qry="select created_by_info,count(1) from m_infra where publisher like '%".getUserID()."%' group by created_by_info";
					$rsFilter=$ora->sql_fetch($qry,$bit_app["db"]);
					for ($i=1;$i<=$rsFilter->jumrec;$i++) {
						$arrData=explode("/",$rsFilter->value[$i][1]);
