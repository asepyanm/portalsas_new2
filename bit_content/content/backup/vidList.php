...
?>
	...
	<tbody>
		<?
			...
			//=============================================
			$qry="select 
					id,keterangan,file,date_format(updated_date,'%Y%m%d, %h:%i:%s') updated_date,funcGetUser(updated_by) updated_by
				  from m_video a
				  where 1=1 $where
				  $orderBy
		...
		?>			
				<tr class="<? echo $cl?>">
					...
					<td width="1%" align="center">
						<img src="<? echo $bit_app["path_url"]?>/bit_images/edit.gif" onClick="var win; win=window.open('<? echo $bit_app["path_url"]."/bit_content/video/"?>edit.php?id=<? echo $rs->value[$i][1]?>','win','top=10,left=100,height=450,width=600,resizable=1,scrollbars=1');win.focus()">
					</td>
					<td width="1%" align="center">
						<img src="<? echo $bit_app["path_url"]?>/bit_images/hapus.png" onClick="var ans;ans=confirm('Anda yakin untuk menghapus  ?'); if (ans) {document.forms[0].hdAction.value=3;document.forms[0].id.value=<? echo $rs->value[$i][1]?>;document.forms[0].submit();}">
					</td>
					<td width="1%" align="center">
						<img src="<? echo $bit_app["path_url"]?>/bit_images/detail.gif" onClick="var win; win=window.open('<? echo $bit_app["path_url"]."/bit_content/video/"?>det.php?id=<? echo $rs->value[$i][1]?>','win','top=10,left=100,height=350,width=600,resizable=1,scrollbars=1');win.focus()">
					</td>
				</tr>	
		<? } ?>
	</tbody>
