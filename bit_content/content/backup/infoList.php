...
?>
	...
	<tr>
		<td colspan="5">
			<? echo setTitleCari("Info Management");?>
		</td>
	</tr>
	</table>
	...
	<thead>
		<tr>
		<? $arrField=array("NO","Title","Tanggal") ?>
		<?
	...
	<tbody>
		<?
			...
			$qry="select 
					id,title,content,date_format(created_date,'%d-%m-%Y'),publish_flag
				  from m_info a
				  where 1=1 $where
				  $orderBy
		...
		?>			
				<tr class="<? echo $cl?>">
					...
					<td width="1%" align="center">
						<img title="Edit Info" style="cursor:pointer" src="<? echo $bit_app["path_url"]?>/bit_images/edit.gif" onClick="var win; win=window.open('<? echo $bit_app["path_url"]."/bit_content/info/"?>edit.php?id=<? echo $rs->value[$i][1]?>','win','top=10,left=100,height=400,width=600,resizable=1,scrollbars=1');win.focus()">
					</td>
					<? if (!$rs->value[$i][5]) { ?>
					<td width="1%" align="center">
						<img title="Enable Info" style="cursor:pointer" src="<? echo $bit_app["path_url"]?>/bit_images/checked_out.png" onClick="var ans;ans=confirm('Anda yakin untuk UnLock Info [<? echo $rs->value[$i][2]?>] ?'); if (ans) {document.forms[0].hdAction.value=4;document.forms[0].status.value='<? echo $rs->value[$i][5]?>';document.forms[0].id.value=<? echo $rs->value[$i][1]?>;document.forms[0].submit();}">
					</td>
					<? } else {?>
					<td width="1%" align="center">
						<img title="Disabled Info" style="cursor:pointer" src="<? echo $bit_app["path_url"]?>/bit_images/tick.png" onClick="var ans;ans=confirm('Anda yakin untuk Lock Info [<? echo $rs->value[$i][2]?>] ?'); if (ans) {document.forms[0].hdAction.value=4;document.forms[0].status.value='<? echo $rs->value[$i][5]?>';document.forms[0].id.value=<? echo $rs->value[$i][1]?>;document.forms[0].submit();}">
					</td>
					<? } ?>
				</tr>	
		<? } ?>
	...
	<hr class="article_layout_hr">
	<p align="right">
	<? $f->button("add","Tambah","button","var win;win=window.open('".$bit_app["path_url"]."/bit_content/content/infoAdd.php','win','top=10,left=100,height=400,width=600,resizable=1,scrollbars=1');win.focus()")?>
