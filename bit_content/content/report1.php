	...
	<table cellpadding="1" cellspacing="0">
		...
		<tr>
			...
			<td align="left">
				: <?
					unset($arrAuthor);
					$qry="select created_by_info,count(1) from m_foto where publisher like '%".getUserID()."%'  group by created_by_info";
					$rsFilter=$ora->sql_fetch($qry,$bit_app["db"]);
					for ($i=1;$i<=$rsFilter->jumrec;$i++) {
						$arrData=explode("/",$rsFilter->value[$i][1]);
		...
		?>			
				<tr class="<? echo $cl?>">
					...
					<td width="1%" align="center">
						<img title="Detail " style="cursor:pointer" src="<? echo $bit_app["path_url"]?>/bit_images/detail.gif" onClick="var win; win=window.open('<? echo $bit_app["path_url"]."/bit_content/foto/"?>det.php?id=<? echo $rs->value[$i][1]?>','win','top=10,left=100,height=650,width=600,resizable=1,scrollbars=1');win.focus()">
					</td>
				</tr>	
		<? } ?>
	</tbody>
