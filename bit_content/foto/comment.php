<?
	...
	if ($_POST["hID"]) {
		$qry="delete from m_foto_comment where id='".$_POST["hID"]."' and foto_detail_id=".$idChild;
	...
	if ($_POST["tComment"] && $_POST["hID"]=="") {
		while (list($k,$v)=each($_POST["tComment"])) {
			if ($v) {
				$qry="insert into m_foto_comment(comment,updated_date,updated_by,foto_id,foto_detail_id)
						values('$v',sysdate(),'".getUserID()."','$k','".$idChild."')";
				$ora->sql_no_fetch($qry,$bit_app["db"]);
			}
		}
	}
	...
	echo "<tr><td align='right' class='bit_back' colspan=3>";
	...
	for ($i=(($_POST["slPage"]-1)*$sumOfPage)+1;$i<=$maxCount;$i++) {
		if ($i!=$page)
			echo "&nbsp;<a class='bit_paging' onClick='document.frContent.page.value=".$i.";dtComment(".$id.",".$idChild.");'>$i</a>&nbsp;";	
		else
	...
	echo "<tr>
			...
			<tr>
				<td  colspan=3 valign='top' align='left'>
					...
					<br />
					<input type='button' value='Reply' name='bComment' onClick='dtComment($id,$idChild);'>
				</td>
			</tr>";
	}
	echo "</table>";
